...
{	
  if(atObject_IDMs_Data.IDM_number >= 1)
  {
      ...
      {     
        if(mb_TCP.isConnected(IP_module))
          ...
          for( uint8_t try_read = 0; try_read <= 2; try_read++)
          {
            ...
            mb_TCP.readHreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP,IDM_REGISTER_RW_INVERTER_TYPE, 
                                                        &atObject_IDMs_Data.IDM[atApp_IDM.number_device].invt_type);
            mb_TCP.readCoil(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_RW_DIRECTION_RUN,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].direction_run);
            mb_TCP.readHreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_RW_SET_FREQUENCY,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].set_frequency);
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_OPERATION_FREQUENCY,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].operating_frequency);
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_INPUT_VOLTAGE,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].input_voltage);  
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_INPUT_CURRENT,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].input_current);
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_OUTPUT_VOLTAGE,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].output_voltage); 
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_OUTPUT_CURRENT,
                                                          &atObject_IDMs_Data.IDM[atApp_IDM.number_device].output_current);
            mb_TCP.readIreg(atObject_IDMs_Data.IDM[atApp_IDM.number_device].IP, IDM_REGISTER_R_FAULT_CODE,
    ...
    if(atApp_IDM.User_Mode == APP_USER_MODE_DEBUG)
    {
      Serial.printf(" - Module IDM %d \n",atApp_IDM.number_device);
      Serial.print("Temperature:"); 
			Serial.print(atObject_IDMs_Data.IDM[atApp_IDM.number_device].Status_of_IDMs);
      Serial.print("\n");
