...
static void Menu_Screen_btn_OK_event_handler(lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
        ...
        {
			lv_scr_load_anim(*Forward_Menu_Screen, LV_SCR_LOAD_ANIM_OVER_LEFT, 100, 100, true);
        }
	}
		break;
	default:
...
static void Menu_Screen_btn_BACK_event_handler(lv_event_t *e)
{
	...
	case LV_EVENT_CLICKED:
	{
		...
		lv_disp_t * d = lv_obj_get_disp(lv_scr_act());
		if (d->prev_scr == NULL && d->scr_to_load == NULL)
			lv_scr_load_anim(*Back_Menu_Screen, LV_SCR_LOAD_ANIM_OVER_RIGHT, 100, 100, true);
	}
		break;
	default:
		break;
